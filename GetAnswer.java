...
class Answer
{
	public String getAnswer(String str)
	{
		...
		{  if(str.charAt(i)=='(')
		{
			...
			String valuestr=getAnswer(s);//对于有括号的，将括号内的部分先进行计算 
	...
	{   
			...
			if(str.charAt(i)=='*'||str.charAt(i)=='/')  //先搜索乘除号位置
	{   int num1=0,num2=0;
	    int len1=0,len2=0;
		for(int j=i-1;j>=0;j--)
	{   
		
		if(str.charAt(j)=='+'||str.charAt(j)=='-')//遇到符号停止
		...
		if(str.charAt(i)=='*')  //若*前的式子里有/，则以分子乘以*后的数，没有则正常计算
		{ int t;
		...
		if(str.charAt(i)=='/') //若/前式子里有/，则分母乘以/后的数
		{   int t;
			if((t=index.indexOf("/"))!=-1)
	...
	public int getnum2(int m,int n)//辗转相除求最大公约数
	{ 
		while(true)
	 {   int r=m%n;
		if(r==0)
...
public class GetAnswer {
	...
	{   Answer answer=new Answer();
		String s="1+2*(6*2)";
	   System.out.println(answer.getAnswer(s));
